...
'   2. 插入 > 模块，将此代码粘贴进去
'   3. 修改下方 OUTPUT_DIR 常量（或运行时会弹窗让你选择目录）
...
Private Function ExportPages(doc As Visio.Document, outputDir As String) As Long
    ...
    For Each page In doc.Pages
        ...
        svgPath = outputDir & safeName & ".svg"

        ' 若同名文件已存在则加序号避免覆盖
        Dim finalPath As String
        finalPath = UniqueFilePath(svgPath)

        ' 导出为 SVG（visSaveAsTypeDefault = 0，格式由扩展名决定）
        page.Export finalPath
